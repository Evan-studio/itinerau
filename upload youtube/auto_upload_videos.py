...
def get_site_url():
            ...
            with open(translations_csv, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    if row.get('key', '').strip() == 'site.domain':
                        # Chercher dans toutes les colonnes possibles (en, fr_auto, etc.)
                        url = None
                        for col in row.keys():
    ...
    # Fallback
    return "https://itinerau.com"

def load_tracking():
    """Charge le fichier de suivi des uploads pour éviter les doublons."""
    if not TRACKING_FILE.exists():
        return set()  # Retourner un set vide au lieu d'un dict
    try:
        import json
...
def build_description(product_id, description_short, site_url):
    ...
    """
    # Construire l'URL de la page produit (sans préfixe /fr car le français sera à la racine après migration)
    ...
    description = f"Visitez notre site pour plus de détails : {product_url}\n\n"
    description += clean_desc if clean_desc else "Détails du produit disponibles sur notre site."
...
def get_csv_delimiter():
    """Détecte le délimiteur du CSV (; ou ,)."""
...
def save_csv_data(df):
    ...
    try:
        ...
        # Utiliser le même délimiteur que le CSV original
        delimiter = get_csv_delimiter()
...
def main():
    ...
    for product_dir in sorted(IMAGES_DIR.iterdir()):
        ...
        # Récupérer les infos du CSV (priorité aux colonnes françaises)
        row = product_row.iloc[0]
        
        # Chercher le titre dans l'ordre: name_fr_auto > titre_fr_auto > name > titre
        name = ''
        if 'name_fr_auto' in df.columns and pd.notna(row.get('name_fr_auto')):
            name = str(row['name_fr_auto']).strip()
        elif 'titre_fr_auto' in df.columns and pd.notna(row.get('titre_fr_auto')):
            name = str(row['titre_fr_auto']).strip()
        elif 'name' in df.columns and pd.notna(row.get('name')):
            name = str(row['name']).strip()
        elif 'titre' in df.columns and pd.notna(row.get('titre')):
            name = str(row['titre']).strip()
        
        # Chercher la description dans l'ordre: description_short_fr_auto > description_short
        description_short = ''
        if 'description_short_fr_auto' in df.columns and pd.notna(row.get('description_short_fr_auto')):
            description_short = str(row['description_short_fr_auto']).strip()
        elif 'description_short' in df.columns and pd.notna(row.get('description_short')):
            description_short = str(row['description_short']).strip()
