...
def load_translations_from_csv():
    ...
    try:
        with open(TRANSLATIONS_CSV, 'r', encoding='utf-8') as f:
            ...
            # Si la première ligne contient un point-virgule, utiliser point-virgule
            delimiter = ';' if ';' in first_line and first_line.count(';') > first_line.count(',') else ','
...
def update_favicon_absolute(html, translations):
    """Met à jour la favicon avec un chemin relatif (pour local) ou absolu (pour production)."""
    # Après migration, le français est à la racine, donc toujours utiliser un chemin relatif
    # Détecter si on est dans un dossier de langue (ex: fr/)
    base_name = BASE_DIR.name.lower()
    is_language_dir = len(base_name) == 2 and base_name.isalpha()
    
    # Si on est dans un dossier de langue, utiliser un chemin relatif
    if is_language_dir:
        favicon_url = "../images/favicon/favicon.ico"
    else:
        # Sinon (à la racine), utiliser un chemin relatif également
        favicon_url = "images/favicon/favicon.ico"
...
<link rel="apple-touch-icon" href="{escape_html_attr(favicon_url)}">'''
    ...
    print(f"  ✅ Favicon mise à jour: {favicon_url}")
    return html
...
def update_footer(html, translations):
    ...
    if sitemap_link:
        footer_links_html.append(f'<a href="sitemap-fr.xml">{escape_html_attr(sitemap_link["text"])}</a>')
...
def update_buttons(html, translations):
    ...
    if button_text:
        # Remplacer les anciennes versions du texte
