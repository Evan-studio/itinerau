...
def detect_language_code():
    """Détecte le code de langue depuis le nom du dossier (fr, de, es, etc.) ou depuis translations.csv."""
    ...
    if len(base_name) == 2 and base_name.isalpha():
        return base_name
    # Sinon, vérifier si translations.csv contient 'fr_auto' (après migration, le français est à la racine)
    if TRANSLATIONS_CSV.exists():
        try:
            with open(TRANSLATIONS_CSV, 'r', encoding='utf-8') as f:
                ...
                if 'fr_auto' in (reader.fieldnames or []):
                    return 'fr'
        except Exception:
            pass
    # Par défaut anglais
...
def update_favicon_absolute(html, translations):
    """Met à jour la favicon avec un chemin relatif depuis page_html/products/."""
    # Depuis page_html/products/, le chemin relatif vers images/favicon/ est ../../../images/favicon/favicon.ico
...
def get_footer_items_from_csv(translations, page_type='product'):
    ...
    if sitemap_key:
        ...
        if sitemap_text:
            footer_items.append({'text': sitemap_text, 'url': '../../sitemap-fr.xml'})
...
def extract_images_from_csv(image_paths_str, product_id):
    """Extrait les 6 premières images depuis le CSV et les convertit en chemins relatifs.
    Vérifie que les fichiers existent réellement dans images/products/ avant de les ajouter."""
    ...
    for i, img_path in enumerate(images):
        if i >= 6:  # Limiter aux 6 premières images
            break
        img_path = img_path.strip()
        if not img_path:
            continue
        
        # Chercher les images avec des noms standardisés : image_1.webp, image_2.webp, etc.
        # C'est le format utilisé dans images/products/
        found_filename = None
        
        # Essayer avec les noms standardisés image_N.ext (priorité aux formats webp, jpg)
        for ext in ['.webp', '.jpg', '.jpeg', '.png']:
            standard_name = f"image_{i+1}{ext}"
            test_file = images_dir / standard_name
            if test_file.exists():
                found_filename = standard_name
                break
        
        # Si on a trouvé un fichier, l'ajouter à la liste
        if found_filename:
            # Chemin relatif depuis page_html/products/ vers images/products/
            # Les images sont dans le dossier parent, donc ../../../images/
            relative_path = f"../../../images/products/{clean_product_id}/{found_filename}"
            result.append(relative_path)
    
    # Si aucune image trouvée, chercher une image par défaut dans le dossier
    if not result:
        if images_dir.exists():
            # Chercher image_1 avec différentes extensions
            for ext in ['.webp', '.jpg', '.jpeg', '.png']:
                default_file = images_dir / f"image_1{ext}"
                if default_file.exists():
                    return [f"../../../images/products/{clean_product_id}/image_1{ext}"]
            
            # Chercher n'importe quel fichier image dans le dossier
            for ext in ['.webp', '.jpg', '.jpeg', '.png']:
    ...
    # Si aucune image trouvée, retourner image_1.webp comme fallback
    # Le navigateur gérera l'erreur avec l'attribut onerror dans le HTML si l'image n'existe pas
    return result if result else [f"../../../images/products/{clean_product_id}/image_1.webp"]
...
def generate_product_page_html(product, translations):
    ...
    domain = get_translation('site.domain', translations, 'https://esport4all.com')
    if domain:
        domain = domain.rstrip('/')
    
    # Construire l'URL de cette page produit
    # Après migration, le français est à la racine, donc toujours utiliser la racine sans /fr/
    ...
    # Après migration, il n'y a qu'une seule langue (français) à la racine
    # Ne pas générer de hreflang puisqu'il n'y a qu'une seule langue
    hreflang_html = ''
    ...
    # Ajouter uniquement le canonical (pas de hreflang puisqu'il n'y a qu'une seule langue)
    ...
    # 3. Modifier le CSS des thumbnails pour afficher 6 images (4 colonnes)
    # Utiliser repeat(4,1fr) pour afficher jusqu'à 6 images (4 colonnes x 2 lignes)
...
'''
    ...
    if affiliate_link:
        button_text = get_translation('button.view_on_aliexpress', translations, 'Voir sur Amazon')
    ...
    if has_youtube and youtube_id:
        domain = get_translation('site.domain', translations, 'https://esport4all.com')
        domain = domain.rstrip('/') if domain else 'https://esport4all.com'
    ...
    for product in products:
        ...
        try:
            ...
            if html:
                ...
                error_count += 1
                print(f"  ❌ Erreur pour produit {clean_product_id}")
        except Exception as e:
            error_count += 1
            print(f"  ❌ Erreur pour produit {clean_product_id}: {e}")
