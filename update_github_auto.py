...
def ensure_remote():
    ...
    try:
        ...
        if success:
            ...
            # Vérifier si le remote pointe déjà vers le bon dépôt (avec ou sans token, SSH ou HTTPS)
            if f"{user}/{repo}.git" in current:
                print_info(f"Remote déjà configuré : {current}")
                # Ne pas changer le remote s'il existe déjà (il peut contenir un token ou être en SSH)
                return True
        
        # Si le remote n'existe pas ou pointe ailleurs, le configurer
