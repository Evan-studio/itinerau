...
def detect_languages():
    ...
    # Langue principale (fr) - dossier racine (après migration, le français sera à la racine)
    if (BASE_DIR / 'index.html').exists() and (BASE_DIR / 'translations.csv').exists():
        # Utiliser le script français pour la racine
        generate_script_fr = BASE_DIR / 'scripts' / 'generate_all_fr.py'
        if generate_script_fr.exists():
            languages.append({
                'code': 'fr',
                'name': 'Français',
                'generate_script': generate_script_fr,
                'update_script': BASE_DIR / 'scripts' / 'generate' / 'update_domain_urls.py',
                'dir': BASE_DIR
            })
        else:
            print(f"⚠️  Script français non trouvé: {generate_script_fr}")
            print(f"   Le site à la racine ne sera pas généré.")
    
    # Autres langues - dossiers avec index.html et translations.csv
    for lang_dir in BASE_DIR.iterdir():
        if lang_dir.is_dir() and not lang_dir.name.startswith('.') and lang_dir.name not in ['scripts', 'CSV', 'upload youtube', 'page_html', 'sauv']:
...
def propagate_youtube_urls_from_root():
    ...
    for lang in LANGUAGES:
        # Le dossier racine (fr après migration) utilise déjà root_csv
        if lang['code'] == 'fr' and lang['dir'] == BASE_DIR:
            continue
...
def run_script(script_path, lang_name, step_name, lang_dir=None):
    """Exécute un script."""
    ...
    try:
        print(f"  📄 {step_name}...")
        # Utiliser lang_dir si fourni, sinon utiliser le dossier parent du script
        cwd_dir = lang_dir if lang_dir else script_path.parent
...
def main():
    ...
    for lang in LANGUAGES:
        ...
        # Étape 1: Génération
        if not run_script(lang['generate_script'], lang['name'], "Génération", lang_dir=lang['dir']):
            print(f"  ⚠️  Échec de la génération pour {lang['name']}")
            continue
        
        # Étape 2: Mise à jour des domaines
        if not run_script(lang['update_script'], lang['name'], "Mise à jour des domaines", lang_dir=lang['dir']):
